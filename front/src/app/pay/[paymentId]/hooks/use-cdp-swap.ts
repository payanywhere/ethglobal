...
import { type CdpSupportedNetwork, estimateSwapPrice } from "@/services/cdp-trade"
...
export function useCdpSwap() {
  ...
  const getRequiredInputAmount = useCallback(
    async ({
      fromToken,
      toToken,
      targetAmountOut,
    ...
    async ({
      ...
      try {
        ...
        if (txData.permit2?.eip712) {
          ...
          transactionData =
            `${transactionData}${signatureLengthHex}${signature.slice(2)}` as `0x${string}`
        }
