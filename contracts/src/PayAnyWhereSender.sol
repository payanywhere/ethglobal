...
contract PayAnyWhereSender {
    using OptionsBuilder for bytes;

    address private constant USDC_ETH_SEPOLIA = 0x4985b8fcEA3659FD801a5b857dA1D00e985863F0;
    uint32 private constant ENDPOINT_ID_POLYGON_AMOY = 40_267;

    function pay() external {
        address stargate = USDC_ETH_SEPOLIA;
        uint32 dstEid = ENDPOINT_ID_POLYGON_AMOY;
        address composer = 0x4985b8fcEA3659FD801a5b857dA1D00e985863F0;
        uint256 amountLD = 10_000;
        uint128 composeGas = 200_000;
        address refund = msg.sender;
        bool payInLzToken = false;

        // Step 1: compose payload
        bytes memory composeMsg = abi.encode(msg.sender, amountLD);
        ...
        sendParam.composeMsg = abi.encode(msg.sender, oftReceipt.amountReceivedLD);
        ...
        if (token != address(0)) {
            _ensureApproval(token, msg.sender, stargate, sendParam.amountLD);
        }
